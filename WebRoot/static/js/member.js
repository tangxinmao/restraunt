...
for(i in serializeArray){
	...
	$.post(root+"/role/saveMemberRole", serializeArray,
			function(j){
	    if(j != undefined && j.code != "1") {
	        alertWarn(j.result);
	        return;
        }
...
function deleteMember(memberId){
	var root = $("#rootsite").val();
	$.post(root+"/role/deleteMemberAndMemberRole", {
		memberId:memberId
	},
			function(j){
		var data=JSON.parse(j);
...
function enable(memberId){
	var root = $("#rootsite").val();
	$.post(root+"/role/enableMemberAndMemberRole", {
		memberId:memberId
	},
			function(j){
		var data=JSON.parse(j);
...
$.post(getRootPath()+"/role/roleMemberRelation",{currentPage:currentPage,
	memberId:memberId,
