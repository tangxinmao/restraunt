...
function descartes(list){  
	...
	while(true){  
		...
		skuHtml=skuHtml+"<td><input type='text' class='text' name='prop_storageNum' maxlength='8' size='10' value='"+storageNum_val+"' onblur=\"skumap.put('storageNum"+sku_val+"',this.value);getSumSkuNum();\"><td><input type='text' class='text' maxlength='8' name='prop_originprice' size='10' value='"+originprice_val+"' onblur=\"skumap.put('originprice"+sku_val+"',this.value);\"></td><td><input name='prop_skuid' type='hidden' value='"+sku_val+"'><input name='prop_skuval' type='hidden' value=''><input type='text' class='text' maxlength='8' name='prop_price' size='10' value='"+price_val+"' onblur=\"skumap.put('price"+sku_val+"',this.value);\"></td></td><td><input type='text' class='text' name='prop_merchantProdId' maxlength='13' size='10' value='"+merchantProdId_val+"' onblur=\"skumap.put('merchantProdId"+sku_val+"',this.value);\"></td><td><a href='javascript:void(0);' onclick='delPropCascade(this)'>Delete</a></td></tr>";
		//压入结果数组  
		result.push(temp);  
		temp = [];  
		//检查指针最大值问题  
	...
	var htmls = "<div class='layout'>"+
			...
			"<div class='main'>"+
				...
				"<div class='freightTemplet_btns mt15'>"+
					"<input type='button' value='OK' onclick=\"javascript:self.location='initBase'\" class='btn btn-sm btn-sgGreen'>"+
				"</div>"+
			"</div>"+
		"</div>";
	$(".shade_layout").show();
...
function innerInit(){
	javascript:self.location='initBase';
}
...
//		          +",,,"+$(this).find("td input[name=prop_price]").val()",,,"+$(this).find("td input[name=prop_merchantProdId]").val();
		...
		if(typeof($(this).find("input[name=prop_storageNum]").val()) != 'undefined'){
			...
			skuvalue[i]=$(this).find("input[name=prop_storageNum]").val()+",,,"+$(this).find("input[name=prop_originprice]").val()+",,,"+$(this).find("input[name=prop_price]").val()+",,,"+$(this).find("input[name=prop_merchantProdId]").val();
