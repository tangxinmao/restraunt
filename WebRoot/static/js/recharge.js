...
function search(){
	...
	var serialize=$('#searchForm').serializeArray();
	var i=0;
	while(i<2){
		if(serialize[i].value&&serialize[i].value!=''){
			break;
		}
		i++;
	}
	if(i==2){
		alertWarn('Search Condition'+' can\'t be empty.');	
	}
/*	$('input[name="memberId"]').val('');*/
	...
	$.post(root+'/memberinfo/validateMobile', $('#searchForm').serializeArray(), function(data) {
				var json=data.data;
				if(json){
			$('#mobile').text(json.mobile);
			$('#memberId').text(json.memberId);
			$('#productListDiv input[name="memberId"]').val(json.memberId);
			$('#name').text(json.name);
			var fomat=formatNum(json.walletAmount?json.walletAmount:'0');
		    $('#balance').text('Rp '+fomat);
		    $('input[name="balance"]').val(json.walletAmount&&json.walletAmount?json.walletAmount:'0');
...
function recharge(){
	var root = $("#rootsite").val();
	if(!$('#productListDiv input[name="memberId"]').val()||$('#productListDiv input[name="memberId"]').val()==''){
		alertWarn('Buyer'+' can\'t be empty.');	
		return;
	}
	if(!$('input[name="amount"]').val()||$('input[name="amount"]').val()==''){
		alertWarn('Top-up Amount'+' can\'t be empty.');	
		return;
	}
	var regInt = /^[0-9]*[1-9][0-9]*$/ ; 
	if(!regInt.test($('input[name="amount"]').val())){
		alertWarn('please input integer about in '+'Top-up Amount'+'!');
		return;}
	var loginMember=JSON.parse($('#loginMember').val());
	$.post(root+'/memberinfo/rechargeWallet', {
		memberId:$('#productListDiv input[name="memberId"]').val(),
		amount:$('input[name="amount"]').val(),
		rechargeStationId:$('input[name="rechargeStationId"]').val()
		}, function(data) {
				if(data.code=='1'){
				var int=parseInt( $('input[name="memberAmount"]').val())+parseInt( $('input[name="walletVal"]').val());
				var fomat=formatNum(int);
				$('input[name="walletVal"]').val('');
				$('input[name="memberAmount"]').val(int);
				$('#memberAmount').text('Rp '+  fomat);
				//friendlyPrompt("success");
				$("#productListDiv").hide();
				$(".searchArea").hide();
				$("#screen3").show();
			//	location.reload();
	...
	var htmls = "<div class='layout'>"+
			...
			"<div class='main'>"+
				...
				"<div class='freightTemplet_btns mt15'>"+
					"<input type='button' style='background-color:#27d0d8' value='OK' onclick=\"javascript:location.reload();\" class='btn btn-sm btn-sgGreen'>"+
				"</div>"+
			"</div>"+
		"</div>";
	$(".shade_layout").show();
...
if($('#payAmountForm input[name="orderPayPrice"]').val()&&$('#payAmountForm input[name="orderPayPrice"]').val()!=''){
	var regInt = /^[0-9]*[1-9][0-9]*$/ ; 
	if(!regInt.test($('#payAmountForm input[name="orderPayPrice"]').val())){
...
if($('#rejectedTransactionForm input[name="refundReason"]:checked').val()&&$('#rejectedTransactionForm input[name="refundReason"]:checked').val()!=''){
	$.post(root+"/order/refundForm", $('#rejectedTransactionForm').serializeArray(),
			function(data){
			if(data.code=="1"){
				close();
...
$(function() {
	setInterval(function(){
		now.innerHTML='Top-up Time: '+new Date().format("yyyy-MM-dd hh:mm:ss");
	},1000);
	var loginMember=JSON.parse($('#loginMember').val());
	
	loginMemberNickname.innerHTML='Top-up Operator: '+loginMember.account;
...
var json={currentPage:currentPage,
		orderStatusStr:orderStatusStr,
		orderTimeStart:orderTimeStart?orderTimeStart+' 00:00:00':'',
		orderTimeEnd:orderTimeEnd?orderTimeEnd+' 23:59:59':'',
		orderId:orderId,
		mobile:mobile,
		city:city,
		orderTimeDesc:orderTimeDesc,
		orderPayPriceDesc:orderPayPriceDesc,
...
var city=$('#searchForm input[name="city"]').val();
var merchantName=$('#searchForm input[name="merchantName"]').val();
$.post(getRootPath()+"/order/orderIndex",{currentPage:currentPage,
	orderStatusStr:orderStatusStr,
	orderTimeStart:orderTimeStart?orderTimeStart+' 00:00:00':'',
	orderTimeEnd:orderTimeEnd?orderTimeEnd+' 23:59:59':'',
	orderId:orderId,
