 ...
 * 验证及提示框
 */
function alertWarnWarn(message){
...
function addMember(){
	...
	var serializeArray=$('#memberWinForm').serializeArray();
for(i in serializeArray){
	if(serializeArray[i].name=='memberId'||serializeArray[i].name=='description')
		continue;
	var text=$('#memberWinForm input[name="'+serializeArray[i].name+'"]').prev('lebel').children().last().text().replace(':','');
	if(text=='')
		text=$('#memberWinForm textarea[name="'+serializeArray[i].name+'"]').prev('lebel').children().last().text().replace(':','');
	if(text=='')
		text=$('#memberWinForm select[name="'+serializeArray[i].name+'"]').prev('lebel').children().last().text().replace(':','');
	...
	$.post(root+"/roleMerchant/saveMemberRole", serializeArray,
			function(j){
...
function deleteMember(memberId){
	var root = $("#rootsite").val();
	$.post(root+"/roleMerchant/deleteMemberAndMemberRole", {
		memberId:memberId
	},
			function(j){
		var data=JSON.parse(j);
...
function enable(memberId){
	var root = $("#rootsite").val();
	$.post(root+"/roleMerchant/enableMemberAndMemberRole", {
		memberId:memberId
	},
			function(j){
		var data=JSON.parse(j);
 ...
 * @param obj DOM对象
 */
function editMemberWin(memberId,obj){

	//$('#memberWinForm input[name="account"]').prop('readonly','readonly').css('border-width','0px');
	$('#memberWin .hd h2').text('Edit User');
	$(".shade_layout").show();
	$("#memberWin").show();
	pop_center ('560',460);
	var $td = $(obj).parent().parent().find("td");
	$('#memberWinForm input[name="memberId"]').val(memberId);
	//权限控制
	var loginMember=JSON.parse($('#loginMember').val());
	if(loginMember.roleId=='1'){
		$('#memberWinForm select[name="roleId"]').find('option').each(function(i){
	...
	$('#memberWinForm select[name="roleId"]').val($td.eq(4).text());
	if($td.eq(4).text()==4||$td.eq(4).text()==5){
		$('#memberWinForm select[name="roleId"]').find('option').hide();
	}
	$('#memberWinForm input[name="account"]').val($td.eq(1).text());
	$('#memberWinForm input[name="password"]').val(null);
	$('#memberWinForm input[name="password"]').prev('lebel').children().first().hide();
	$('#memberWinForm input[name="password"]').prev('lebel').children().last().text('Reset Password:');
	$('#memberWinForm input[name="password"]').prev('lebel').css('margin-left','-44px');
	$('#memberWinForm input[name="email"]').val($td.eq(2).text());
	$('#memberWinForm textarea[name="description"]').val($td.eq(3).text());
...
/**
	 * 父分类弹出层
	 */
	function addMemberWin(){
		$('#memberWinForm')[0].reset();
		$('#memberWinForm input[name="memberId"]').val(null);
		$('#memberWin .hd h2').text('Add User');
		//权限控制
		var loginMember=JSON.parse($('#loginMember').val());
		console.log(loginMember)
		if(loginMember.roles[0].roleId==1){
			$('#memberWinForm select[name="roleId"]').find('option').each(function(i){
				if($(this).val()=='4'||$(this).val()=='5'){
					$(this).hide();
				}else{
		...
		$('#memberWinForm input[name="account"]').removeProp('readonly').css('border-width','1px');
		$('#memberWinForm input[name="password"]').prev('lebel').children().first().show();
		$('#memberWinForm input[name="password"]').prev('lebel').children().last().text('Password:');
		$('#memberWinForm input[name="password"]').prev('lebel').css('margin-left','-13px');
...
var goThisPage=function(ta) {
	...
	var memberId=$('#searchForm input[name="memberId"]').val();
	var memberAccount=$('#searchForm input[name="memberAccount"]').val();
	var roleId=$('#searchForm [name="roleId"]').val();
	var email=$('#searchForm [name="email"]').val();
	
$.post(getRootPath()+"/roleMerchant/roleMemberRelation",{currentPage:currentPage,
	memberId:memberId,
