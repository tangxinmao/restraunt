...
for(i in serializeArray){
	...
	$.post(root+"/roleMerchant/updateRole", serializeArray,
			function(data){ 
...
function addMenu(){
	...
	$.post(getRootPath()+"/roleMerchant/menuRole", serializeArray,
			function(data){
		var json =JSON.parse(data);
			if(json.code=="1"){
				 close();
...
function editPermissionsWin(roleId,obj){
	...
	//构建菜单table
	$.post(getRootPath()+'/roleMerchant/menuCheckedByRole',{roleId:roleId},function(data){
...
$.post(getRootPath()+"/roleMerchant/roleMemuRelation",{currentPage:currentPage,
