...
var goThisPage=function(ta) {
	...
	if(ta == 'search'){
		...
		currentPage = 1;
		$("#prodStatus").find("option[value='3']").prop("selected",true);
	}else if(ta == 'sort'){
		currentPage = 1;
		if($("#isDesc").val()=="0"){
			isDesc = "1";
	...
	function(data){
		...
		if(prodStatus==""){
			...
			$(".sortArea ul li").eq(3).addClass("selcted");
		}
...
function beginremoveGoods(prodId){
	var currentPage = $("#currentPage").val();
	var url = getRootPath()+"/product/prod_put_up_down";
	$.post(url,{"prodId":prodId,"action":"remove","currentPage":currentPage},function(data){
		goThisPage('search');
		var status = $("#status_hide").val();
		$("select option").each(function (){  
			if($(this).val()==status){   
...
function batsRemove(){
	...
	showConfirm('Are you sure to put to out of stock?','beginremoveGoods(\''+prodIds+'\')');
}
